/**
 * Project Context for DyDo's Claude Code Planning
 *
 * Manages cached project context that DyDo uses to:
 * - Understand project structure before spawning Claude Code
 * - Make informed decisions during Claude Code sessions
 * - Learn and remember user preferences per project
 */
...
const log = createSubsystemLogger("claude-code/project-context");

/** Base directory for DyDo's project contexts */
const PROJECTS_BASE = "/Users/dydo/clawd/projects";

/** How old context can be before it's considered stale (7 days) */
...
export function isContextStale(context: ProjectContext): boolean {
  ...
  return Date.now() - lastExplored > STALENESS_THRESHOLD_MS;
}
...
function detectProjectType(projectPath: string): string | undefined {
  ...
  if (baseType === "Node.js") {
    const hasReact =
      ...
      fs.existsSync(path.join(projectPath, "app/page.tsx")); // Next.js
...
function detectBuildTool(projectPath: string): string | undefined {
  ...
  // TypeScript compiler as build tool
...
export function listProjectsWithContext(): string[] {
  ...
  return fs.readdirSync(PROJECTS_BASE).filter((name) => {
    const contextPath = path.join(PROJECTS_BASE, name, "context.yaml");
...
export function formatContextForPrompt(context: ProjectContext): string {
  ...
  if (context.claudeMd) {
    ...
    lines.push("```");
    lines.push(context.claudeMd.slice(0, 2000)); // Truncate if too long
