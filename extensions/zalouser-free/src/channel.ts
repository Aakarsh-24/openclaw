        ...
        gateway: {
            startAccount: async (ctx: any): Promise<void> => {
                ...
                if (sessionManager.hasSavedCredentials(accountId)) {
                    ...
                    logger.warn?.(`[zalouser-free] Could not restore session for ${accountId}: ${result.error}`);
                } else {
                    logger.warn?.(`[zalouser-free] No saved credentials for ${accountId}. Run: openclaw zalouser-free login`);
                }
            },
